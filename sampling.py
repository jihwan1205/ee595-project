...
def main(args):
    ...
    for nfe in args.nfe_list:
        ...
        for i in tqdm(range(num_batches), desc=f"NFE={nfe}"):
            ...
            # Generate samples
            with torch.no_grad():
                image_resolution = getattr(args, 'image_resolution', 64)
                samples = model.sample((B, 3, image_resolution, image_resolution), num_inference_timesteps=nfe, verbose=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_resolution", type=int, default=64,
                       help="Image resolution (default: 64, use 256 for CelebA-HQ-256)")
